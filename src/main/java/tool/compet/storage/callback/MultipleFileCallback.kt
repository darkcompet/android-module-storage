...
import tool.compet.storage.callback.FileCallback.FileConflictAction
import tool.compet.storage.callback.DirCallback.ConflictResolution
...
abstract class MultipleFileCallback @JvmOverloads constructor(
	 ...
	 * * [DirCallback.ErrorCode.SOURCE_FILE_NOT_FOUND]
	 * * [DirCallback.ErrorCode.STORAGE_PERMISSION_DENIED]
	 * * [DirCallback.ErrorCode.TARGET_FOLDER_CANNOT_HAVE_SAME_PATH_WITH_SOURCE_FOLDER]
	 */
	@UiThread
	open fun onInvalidSourceFilesFound(
		invalidSourceFiles: Map<DocumentFile, DirCallback.ErrorCode>,
	...
	open fun onContentConflict(
		destinationParentFolder: DocumentFile,
		conflictedFiles: MutableList<DirCallback.FileConflict>,
		action: DirCallback.FolderContentConflictAction
	) {
		action.confirmResolution(conflictedFiles)
	}
