...
import tool.compet.storage.FileCreationMode
import kotlinx.coroutines.CancellableContinuation
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.GlobalScope

abstract class DirCallback @JvmOverloads constructor(
	uiScope: CoroutineScope = GlobalScope
) : BaseFileCallback<DirCallback.ErrorCode, DirCallback.Report, DirCallback.Result>(uiScope) {
	...
	enum class ConflictResolution {
		...
		fun toCreateMode() = when (this) {
			REPLACE -> FileCreationMode.REPLACE
			MERGE -> FileCreationMode.REUSE
			else -> FileCreationMode.CREATE_NEW
		}
